...
export const medicationValidationRules = [
  ...
  body('medications.*.weight').isNumeric().withMessage('Weight must be a number'),
...
export const loadingRequestValidationRules = [
  body('medicationIds').optional().isArray().withMessage('medicationIds must be an array'),

  body('medicationIds.*').optional().isString().withMessage('Each medicationId must be a string'),
